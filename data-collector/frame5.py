...
class Frame5:
    ...
    def parse(self, msg):
        ...
        self.dic["pressure2 samples"] = []
        self.dic["pressure2 count"] = struct.unpack('!H',msg[9:11])[0]
        for i in range(self.dic["pressure2 count"]):
            self.dic["pressure2 samples"].append(
                struct.unpack('!H', msg[13+i*2:15+i*2])[0])
        self.dic["average pressure2"] = \
            sum(self.dic["pressure2 samples"]) / self.dic["pressure2 count"]
        self.averager.add(self.name, self.dic["average pressure2"])

        return self.dic

    def get_json_dic(self):
        return {
            "AVERAGEPRESSURE2": self.averager.get_av(self.name)
        }
...
if __name__ == '__main__':
    ...
    frame5 = Frame5(shared)
    frame5.parse(bytes.fromhex(hexstring_list.data[5]))
    frame5.test()
