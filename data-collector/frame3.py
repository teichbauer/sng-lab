...
class Frame3:
    ...
    def parse(self, msg):
        ...
        self.dic["voltage2 samples"] = []
        self.dic["voltage2 count"] = struct.unpack('!H',msg[9:11])[0]
        for i in range(self.dic["voltage2 count"]):
            self.dic["voltage2 samples"].append(
                struct.unpack('!H', msg[13+i*2:15+i*2])[0])
        self.dic["average voltage2"] = \
            sum(self.dic["voltage2 samples"]) / self.dic["voltage2 count"]
        self.averager.add(self.name, self.dic["average voltage2"])
    ...
    def get_json_dic(self):
        return {
            "AVERAGEVOLTAGE2": self.averager.get_av(self.name)
        }
        
    def test(self):
        from pprint import pprint
        pprint(self.dic, indent=4)
        with open('../output_json/frame3.txt', 'wt') as out:
            pprint(self.dic, stream=out)

if __name__ == '__main__':
    from common import shared
    from testdata import hexstring_list
    frame3 = Frame3(shared)
    frame3.parse(bytes.fromhex(hexstring_list.data[3]))
    frame3.test()
